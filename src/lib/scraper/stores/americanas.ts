...
export async function scrapeAmericanas(url: string): Promise<ScrapeResult> {
  ...
  try {
    const html = await fetchPage(url, 'americanas');
    const $ = cheerio.load(html);

    const priceEl = $('[data-testid="price-value"], .price__SalesPrice, .priceSales').first().text();
    if (priceEl) {
    ...
    const oldPriceEl = $('[data-testid="price-original"], .price__ListPrice, .priceList').first().text();
    if (oldPriceEl) base.originalPrice = parsePrice(oldPriceEl);

    const discountEl = $('[data-testid="price-discount"], .price__discount').first().text();
